...
const sizeInput = function() {
	do {
		size = parseInt(prompt("Ներմուծեք դաշտի չափը՝ 3-15"));
...
const print = function(element) {
	setTimeout(function() {alert(`Հաղթեց ${element}-ով խաղացողը`)}, 100);
...
const check = function(e, step) {
	...
	if(step === size * size) {
		setTimeout(function() {alert("Խաղն ավարտվեց ոչ֊ոքի")}, 100);
		setTimeout(reset,200);
	}
}
